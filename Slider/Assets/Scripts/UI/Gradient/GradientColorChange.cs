        ...
        private void RunAnimation()
        {
            sequenceHelper.Sequence(
                DOTween.To(
                    (v) =>
                    {
                        material.SetColor(topColorId,
                            UnityEngine.Color.Lerp(colors[currentColorIndex % colors.Length].topColor,
                                colors[(currentColorIndex + 1) % colors.Length].topColor, v));
            ...
            sequenceHelper.Sequence(
                DOTween.To(
                    (v) =>
                    {
                        material.SetColor(bottomColorId,
                            UnityEngine.Color.Lerp(colors[currentColorIndex % colors.Length].botomColor,
                                colors[(currentColorIndex + 1) % colors.Length].botomColor, v));
                    }, 0, 1, changeColorSpeed),
